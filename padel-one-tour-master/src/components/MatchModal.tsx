...
import { reprocessMatchRating, processMatchRating } from '../lib/ratingEngine';
import { reverseRatingForSource } from '../lib/levelHistory';
...
export default function MatchModal({ tournamentId, tournament, matchId, onClose, onSuccess, isIndividualRoundRobin = false, individualPlayers = [] }: MatchModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Se o jogo já tinha rating aplicado (torneio finalizado) e o resultado mudou,
    // reverter o rating antigo e aplicar o novo com o score atual.
    if (matchId && finalStatus === 'completed') {
      const wasRated = !!matchData?.rating_processed
      const scoresChanged =
        !matchData ||
        (matchData.team1_score_set1 ?? 0) !== (formData.team1_score_set1 ?? 0) ||
        (matchData.team2_score_set1 ?? 0) !== (formData.team2_score_set1 ?? 0) ||
        (matchData.team1_score_set2 ?? 0) !== (formData.team1_score_set2 ?? 0) ||
        (matchData.team2_score_set2 ?? 0) !== (formData.team2_score_set2 ?? 0) ||
        (matchData.team1_score_set3 ?? 0) !== (formData.team1_score_set3 ?? 0) ||
        (matchData.team2_score_set3 ?? 0) !== (formData.team2_score_set3 ?? 0)

      try {
        if (wasRated && scoresChanged) {
          console.log('[MATCH_MODAL] Result changed after rating — reprocessing match', matchId)
          await reprocessMatchRating(matchId)
        } else if (!wasRated && (tournament as any)?.status === 'completed') {
          console.log('[MATCH_MODAL] Tournament already completed — processing rating for match', matchId)
          await processMatchRating(matchId)
        }
      } catch (ratingErr) {
        console.error('[MATCH_MODAL] Rating update after score edit failed:', ratingErr)
      }
    }
  ...
  const handleRevert = async () => {
    ...
    try {
      ...
      // Reverter níveis se este jogo já tinha sido processado pelo rating engine
      if (matchData?.rating_processed) {
        try {
          const reversed = await reverseRatingForSource(matchId)
          console.log(`[MATCH_MODAL] Reverted ${reversed} rating rows for match ${matchId}`)
        } catch (revErr) {
          console.error('[MATCH_MODAL] Failed to reverse rating on revert:', revErr)
        }
      }
