        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2 lg:flex-shrink-0">
            ...
            {currentTournament.status === 'completed' && (
              <button
                onClick={async () => {
                  if (!confirm('Reprocessar ratings e rewards para todos os jogos deste torneio?\n\nOs níveis anteriores destes jogos serão revertidos e recalculados com os resultados atuais.')) return;
                  setLoading(true);
                  try {
                    // 1) Reverter ratings já aplicados (via histórico) antes de reprocessar
                    ...
                    let msg = `↩️ Revertidos: ${reversedTotal} ajustes de nível`;
                    msg += `\n📊 Ratings: ${ratingResult.processed} processados, ${ratingResult.skipped} saltados, ${ratingResult.errors} erros`;
                    msg += `\n🏆 Rewards: ${rewardResult.awarded} atribuídos, ${rewardResult.skipped} saltados, ${rewardResult.errors} erros`;
                    if (rewardResult.details.length > 0) {
