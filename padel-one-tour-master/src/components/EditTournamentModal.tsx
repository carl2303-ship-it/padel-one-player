...
export default function EditTournamentModal({ tournament, onClose, onSuccess, isIndependentOrganizer }: EditTournamentModalProps) {
  ...
  const [dailySchedules, setDailySchedules] = useState<DailyScheduleEntry[]>(((tournament as any).daily_schedules || []) as DailyScheduleEntry[]);
    ...
    const { error: submitError } = await supabase
      ...
      .update({
        ...
        number_of_courts: isLadderFmt
          ? 1
          : effectiveCourtNames.length > 0
            ? effectiveCourtNames.length
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                  <option value="">{(t.tournament as any).noClubOption || '-- Sem clube (organizador independente) --'}</option>
                  {clubs.map((club) => (
              ...
              {!selectedClubId && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    {(t.tournament as any).manualCourts || 'Campos do Torneio'} *
                  </label>
                  <p className="text-xs text-gray-500 mb-3">
                    {(t.tournament as any).manualCourtsHint || 'Adicione os campos onde o torneio vai decorrer.'}
                  </p>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder={(t.tournament as any).courtNamePlaceholder || 'Nome do campo (ex: Campo 1)'}
                      className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    />
                    <button
                      type="button"
                    ...
                    >
                      ...
                      {(t.tournament as any).addCourt || 'Adicionar'}
                    </button>
                  </div>
                  {manualCourtNames.length > 0 && (
                    <p className="text-xs text-green-600 mt-2 font-medium">
                      {manualCourtNames.length} campo(s) adicionado(s)
                    </p>
                  )}
                </div>
              )}
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            ...
            <div className="flex items-center gap-3 mt-3">
              <input
                type="checkbox"
                id="edit_has_dinner_option"
                checked={formData.has_dinner_option}
                onChange={(e) => setFormData({ ...formData, has_dinner_option: e.target.checked })}
                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
              />
              <label htmlFor="edit_has_dinner_option" className="text-sm font-medium text-gray-700">
                🍽️ Opção de jantar
              </label>
            </div>
            <p className="text-xs text-gray-500 mt-1 ml-7">
          ...
          <div className="pt-4 border-t border-gray-200">
            ...
            <button
              ...
              onClick={async () => {
                if (!confirm('⚠️ ATENÇÃO: Isto vai recalcular os ratings de TODOS os torneios!\n\n1. Reverter níveis já aplicados (via histórico)\n2. Reset de contadores (rated_matches, wins, losses)\n3. Reprocessar todos os jogos de torneio por ordem cronológica\n\nContinuar?')) return;
                setReprocessLoading(true);
                try {
                  // Reverse all logged tournament ratings first (prevents double-application on levels)
                  const { data: allRated } = await supabase
                    .from('matches')
                    .select('id')
                    .eq('status', 'completed')
                    .eq('rating_processed', true);
                  const { reverseRatingForSource } = await import('../lib/levelHistory');
                  let reversedTotal = 0;
                  for (const m of allRated || []) {
                    reversedTotal += await reverseRatingForSource(m.id);
                  }
                  ...
                  msg += `Níveis revertidos: ${reversedTotal}\n`;
