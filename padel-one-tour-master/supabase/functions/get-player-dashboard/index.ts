  ...
  try {
    ...
    if (playerIds.length > 0 || teamIds.length > 0) {
      ...
      if (matchConditions.length > 0) {
        ...
        const totalMatches = wins + draws + losses + completedNoScore;
        stats = {
          totalMatches,
          wins,
          draws,
        ...
        console.log('[EdgeFn] Tournament match breakdown: wins=', wins, 'draws=', draws, 'losses=', losses, 'noScore=', completedNoScore, 'total=', totalMatches);
      ...
      if (ogGameIdSet.size > 0) {
        ...
        if (confirmedResults && confirmedResults.length > 0) {
          ...
          const openGameMatches = confirmedResults.map((result: any) => {
            ...
            else if (is_winner === null) openDraws++;
          ...
          const totalDraws = tournamentDraws + openDraws;
          const totalLosses = tournamentLosses + openLosses;
          const decided = totalWins + totalLosses;

          stats = {
            totalMatches: totalWins + totalDraws + totalLosses,
            wins: totalWins,
            draws: totalDraws,
            losses: totalLosses,
          ...
          console.log('[EdgeFn] Open game stats: wins=', openWins, 'draws=', openDraws, 'losses=', openLosses, 'total open results=', confirmedResults.length);
